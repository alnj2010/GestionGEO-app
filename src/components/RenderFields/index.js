    ...
    render(){
      ...
      return children.map((input,index) => {
        switch(input.type){
          ...
          case 'select': return <Grid key={index} item xs={movil} sm={totalFields>1 ? desktop :12} ><Select {...input} /></Grid>
          ...
          case 'time': return <Grid key={index} item xs={movil} sm={desktop}> <Time {...input} /></Grid>
