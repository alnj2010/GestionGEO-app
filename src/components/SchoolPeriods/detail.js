        ...
        return subjects.filter(
            (item) =>
                !subjectsSelected.some(
                    (selected, index) =>
                        selected.subjectId === item.id && pos > index
                )
        );
    };
        ...
        <Fragment>
            {fields.map((subject, index) => (
                <Grid container justify="center" key={index}>
                            ...
                            {[
                                {
                                    ...
                                    type: 'select',
                                    label: 'Materia',
                                    options: this.unselectedSubjects(index).map(
                                        (subject) => {
                                            return {
                                ...
                                },
                                {
                                    ...
                                    options: this.props.teachers.map(
                                        (teacher) => {
                                            return {
                    ...
                    <Grid item xs={10}>
                        <FieldArray
                            name={`${subject}.schedules`}
                            component={this.renderSchedule}
                        />
                    </Grid>
                </Grid>
            ))}
                    ...
                    <Fab
                        ...
                        disabled={
                            ...
                            this.props.subjects.length ===
                                this.props.subjectsSelected.length
                        }
                        onClick={() =>
                            fields.push({
                                schedule: [
...
const schoolPeriodValidation = (values) => {
    ...
    if (!values.endDate) errors.endDate = '*Fecha fin es requerida';
        ...
        if (moment(values.endDate) <= moment(values.startDate))
            errors.endDate =
                '*Fecha fin no debe estar por debajo de la inicial';

        if (moment(values.withdrawalDeadline) <= moment(values.startDate))
            errors.endDate =
                '*Fecha limite de retiro no debe estar por debajo de la inicial';

        if (moment(values.endDate) < moment(values.withdrawalDeadline))
            errors.endDate =
                '*Fecha limite de retiro no debe estar por encima de la final';
    }
            ...
            if (subj.schedules && subj.schedules.length) {
                ...
                subj.schedules.forEach((sche, scheIndex) => {
                    ...
                    if (!sche || !sche.endHour) {
                        ...
                        moment(sche.endHour, 'hh:mm:ss').isBefore(
                            moment(sche.startHour, 'hh:mm:ss')
                        )
                    ) {
                        scheErrors.endHour =
                            '*Hora fin no debe estar por debajo de la inicial';
                        subjErrors.schedules[scheIndex] = scheErrors;
                    }
                });
                subjectArrayErrors[subjIndex] = subjErrors;
...
SchoolPeriodDetail = connect(
    (state) => ({
        initialValues: {
            codSchoolPeriod: state.schoolPeriodReducer.selectedSchoolPeriod
                .cod_school_period
            ...
            withdrawalDeadline: state.schoolPeriodReducer.selectedSchoolPeriod
                .withdrawal_deadline
                ? state.schoolPeriodReducer.selectedSchoolPeriod
                      .withdrawal_deadline
                : moment().add(1, 'days').format('YYYY-MM-DD'),
            subjects: state.schoolPeriodReducer.selectedSchoolPeriod.subjects
                          ...
                          schedules: subj.schedules
                              ? subj.schedules.map((sche) => ({
                                    schoolPeriodSubjectTeacherId:
                                        sche.school_period_subject_teacher_id,
                                    day: sche.day,
                ...
                : [
                      {
                          schedules: [
                              { endHour: '00:00:00', startHour: '00:00:00' },
                          ],
                      },
                  ],
        },
