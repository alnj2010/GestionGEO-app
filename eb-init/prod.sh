...
if [ -z $PROD_APPLICATION_ENV ];then
  echo "Please set the environment variable PROD_APPLICATION_ENV in your repository settings (Settings/Pipelines/Environment Variables)"
  exit 1
fi
...
global:
  ...
  profile: eb-cli
  sc: git
" $PROD_APPLICATION_ENV $APPLICATION_NAME $AWS_DEFAULT_REGION > /opt/atlassian/pipelines/agent/build/.elasticbeanstalk/config.yml